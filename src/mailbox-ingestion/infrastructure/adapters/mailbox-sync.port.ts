...
export interface MailboxSyncPort {
  establishWatch(mailboxId: string, accessToken: string): Promise<{ subscriptionId: string; expiresAt: Date }>;
  renewWatch(mailboxId: string, subscriptionId: string, accessToken: string): Promise<{ expiresAt: Date }>;
  pullDelta(mailboxId: string, accessToken: string, cursorValue: string): Promise<{ messages: string[]; newCursor: string }>;
